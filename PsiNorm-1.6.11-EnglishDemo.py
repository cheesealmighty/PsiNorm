...
def guiPatientInfo():
    import tkinter as tk
    titleText = "PsiNorm Percentile Calculator - Patient Info"
    patient_sex = None

    class patientInfo:
        def __init__(self, master):
            self.master = master
            master.title("PsiNorm Percentile Calculator")
            
            self.label = tk.Label(master, text=titleText, relief=tk.GROOVE, font= (None, 16))
            self.label.grid(row=0, column=0, padx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=5)    

            self.label = tk.Label(master, text="Test Administrator: ", relief=tk.GROOVE)
            self.label.grid(row=1, column=0, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=1)    
            ...
            self.label = tk.Label(master, text="Patient ID: ", relief=tk.GROOVE)
            self.label.grid(row=2, column=0, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=1)    
            ...
            self.label = tk.Label(master, text="Patient Name: ", relief=tk.GROOVE)
            self.label.grid(row=3, column=0, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=1)    
            ...
            self.label = tk.Label(master, text="Patient Age: ", relief=tk.GROOVE)
            self.label.grid(row=4, column=0, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=1)    
            ...
            self.label = tk.Label(master, text="Patient Sex: ", relief=tk.GROOVE)
            self.label.grid(row=5, column=0, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=1)    
         
            self.buttonFemale = tk.Button(master, text="Female", command=self.buttonFemaleFunc, state = (tk.DISABLED if patient_sex == "Kadın" else tk.NORMAL))
            self.buttonFemale.grid(row=5, column=1, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W) 
            
            self.buttonMale = tk.Button(master, text="Male", command=self.buttonMaleFunc, state = (tk.DISABLED if patient_sex == "Erkek" else tk.NORMAL))
            ...
            self.label = tk.Label(master, text="Total years of education: ", relief=tk.GROOVE)
            self.label.grid(row=6, column=0, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W, columnspan=1)    
            ...
            self.buttonSave = tk.Button(master, text="Save", command=self.saveFunc)
            self.buttonSave.grid(row=7, column=1, ipadx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W) 
 
            self.buttonAbort = tk.Button(master, text="Cancel", command=self.abortFunc)
        ...
        def saveFunc(self):
            ...
            if not checkGuiInputString(patient_admin):
                self.label1.config(text = "Error, invalid input.")
                inputError = True
            else:
                self.label1.config(text = "*")
                
            if not checkGuiInputString(patient_ID):
                self.label2.config(text = "Error, invalid input.")
                inputError = True
            else:
                self.label2.config(text = "*")
                
            if not checkGuiInputString(patient_name):
                self.label3.config(text = "Error, invalid input.")
                inputError = True
            else:
                self.label3.config(text = "*")
                
            if not checkGuiInputInt(patient_age):
                self.label4.config(text = "Error, invalid input.")
                inputError = True
            else:
            ...
            if str(self.buttonFemale["state"]) == "disabled" or str(self.buttonMale["state"]) == "disabled":
                ...
                self.label5.config(text = "Error, please enter all data required.")
                inputError = True
                
            if not checkGuiInputInt(patient_edu):
                self.label6.config(text = "Error, invalid input.")
                inputError = True
            else:
        ...
        def on_saving(self, temppatient_admin, temppatient_ID, temppatient_name, temppatient_age, temppatient_sex, temppatient_edu):            
            text_dump = ("Test administrator: " + temppatient_admin + "\nPatient ID: " + temppatient_ID +
            "\nPatient name: " + temppatient_name + "\nPatient age: " + str(temppatient_age) +
            "\nPatient sex: " + temppatient_sex + "\nTotal years of education: " + str(temppatient_edu)+
            "\nDo you confirm this data?")
            if tk.messagebox.askokcancel("Confirm", text_dump):
...
def guiStartupMenu():
    ...
    from tkinter import messagebox
    import sys
    titleText = " PsiNorm Percentile Calculator - 1.6.11"
 ...
 """
    ...
    class MyFirstGUI:
        def __init__(self, master):
            ...
            self.greet_button = tk.Button(master, text="Start", command=self.startup)
            self.greet_button.grid(row=4, column=0, padx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W) 
    
            self.greet_button = tk.Button(master, text="FAQ", command=self.guiFAQ)
            self.greet_button.grid(row=4, column=1, padx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W) 
            
            self.greet_button = tk.Button(master, text="About", command=self.guiAbout)
            self.greet_button.grid(row=4, column=2, padx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W) 
            
            self.greet_button = tk.Button(master, text="References", command=self.guiReferences)
            self.greet_button.grid(row=4, column=3, padx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W)
            
            self.greet_button = tk.Button(master, text="Settings", command=self.menuSettings)
            self.greet_button.grid(row=4, column=4, padx=10, pady=10, sticky=tk.N+tk.S+tk.E+tk.W) 
            
            self.greet_button = tk.Button(master, text="Exit", command=self.programExit)
...
def mainMenu():
    ...
    menu_ui_layout = """
|>=============================================|=====================================================<|
|        WARNING >>> TO SAVE YOUR DATA, DON'T FORGET TO ENTER """ +  str(len(menu_list)+1) + """                          <<< WARNING |
|      WARNING >>>  AFTER YOU ENTER THE SAVE COMMAND, WAIT FOR THE PROGRAM TO SAVE.   <<< WARNING     |
|>=============================================|=====================================================<|     
|                                              |                                                      | 
|>=============================================|=====================================================<|
|A. General Purpose Tests                      |B. Attention Tests                                    |
...
|                                              |                                                      |
|>=============================================|=====================================================<|
|C. Memory Tests                               |D. Executive Function Tests                           |
|(5)Artırılmış İpuçlu Hatırlama       {4!s:^8} |(8)Trailmaking                               {7!s:^8} |
...
|>=============================================|=====================================================<|
|E. Language Tests                             |F. Visuospatial Function Tests                        |
|(15)Sözel Akıcılık                  {14!s:^9} |(7)Rey Karmaşık Figür                        {6!s:^8} |
|(16)Semantik Akıcılık               {15!s:^9} |(17)Clock Drawing                           {16!s:^9} |
...
|>=============================================|=====================================================<|
|           LEGEND: (-) means that test has not been entered, (+) means it has been entered.          |
|                If you have entered a test twice, it marks with with a (!+!) symbol.                 |
|>=============================================|=====================================================<|
|                                              |                                                      |
|>=============================================|=====================================================<|
|                            To fill up a patient information form, enter 888.                        |
|>=============================================|=====================================================<|
|         WARNING >>> TO SAVE YOUR DATA, DON'T FORGET TO ENTER """ +  str(len(menu_list)+1) + """                          <<< WARNING |
|      WARNING >>>  AFTER YOU ENTER THE SAVE COMMAND, WAIT FOR THE PROGRAM TO SAVE.   <<< WARNING     |
|>=============================================|=====================================================<|   
"""
    ...
    while True:
        try: 
            ...
            menu_input = numInput("Input the test number you want to input. To save your data enter (" + str(len(menu_list)+1) + ") : " )   
